...
def get_simple_task_and_minimize(cnfs):
    n = len(cnfs[0])
    cnt = 0
    vars = list(map(lambda x: abs(x), cnfs[0]))
    clauses = []
    for comb in range(2 ** n):
        ...
        skip = False
        for cnf in cnfs:
            assert len(assumption) == len(cnf)
            eq = True
            for s, v in zip(assumption, cnf):
                eq &= (s == (v < 0))
            if eq:
                skip = True
                break
        if not skip:
            cnt += 1
            clause = []
            for var, s in zip(vars, assumption):
                clause.append(var if s == 1 else -var)
            clauses.append(clause)
    clauses = mini(clauses)
    return clauses
